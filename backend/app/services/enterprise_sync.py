...
class EnterpriseSyncService:
    ...
    async def update_enterprise_info(
        ...
        logger.info(f"Published enterprise_info update: tenant={tenant_id} {info_type} v{info.version}")
        return info

    async def sync_to_agent(self, db: AsyncSession, agent_id: uuid.UUID) -> None:
        ...
        if agent.tenant_id:
            result = await db.execute(
                ...
                .where(EnterpriseInfo.tenant_id == agent.tenant_id)
        ...
        for info in all_info:
            ...
            if info.visible_roles and agent.role_description and agent.role_description not in info.visible_roles:
                continue
            ...
            visible_names.add(file_path.name)

        for existing in agent_dir.glob("*.json"):
            if existing.name not in visible_names:
    ...
    async def sync_to_all_agents(self, db: AsyncSession, tenant_id: uuid.UUID | None = None) -> int:
        ...
        query = select(Agent).where(Agent.status == "running")
        if tenant_id:
        ...
        for agent in agents:
            await self.sync_to_agent(db, agent.id)
